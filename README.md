...
|-------|------|--------------|
| `docs-sync` | Stop hook | If source changed this session but nothing under the docs path did, blocks the stop **once** with a reminder to document the feature. The `stop_hook_active` guard prevents a block loop. |
...
| `feature-docs-init` | skill | One-time per-repo setup: scaffolds the docs dir + index and writes a config only if the defaults don't fit. |
...
To tune it, add `.claude/feature-docs.json` at the repo root (any omitted key
...
- Or set the env vars `FEATURE_DOCS_SRC_GLOB`, `FEATURE_DOCS_SRC_EXCLUDE`,
  `FEATURE_DOCS_DOCS_PATH`, `FEATURE_DOCS_REASON`.

Run the `feature-docs-init` skill once to set this up interactively.
...
- The hook only fires inside a git checkout and only considers working-tree
  changes, so it reflects what you touched this session.
